...
def index_for_glob(glob_string,
                   dir_root=None,
                   index_file=None,
                   replace=False,
                   file_type=None,
    ...
    if os.path.isfile(index_file) and not replace:
        gI = pc.geoIndex().from_file(index_file, read_file=True)
        file_re = re.compile(r'file_\d+')
        have_files=[]
        for key, file in gI.attrs.items():
            if file_re.search(key) is None:
                continue
            have_files.append( os.path.abspath(gI.resolve_path(file)) )
    else:
        gI=pc.geoIndex(delta=delta, SRS_proj4=SRS_proj4)
        have_files = []
    if verbose:
        print(f'index_glob : have {len(have_files)} files')
    files=[]

    for thestr in glob_string.split(' '):
        for thefile in glob.glob(thestr):
            if os.path.abspath(thefile) not in have_files:
                files.append(thefile)

    if verbose:
        print(f'index_glob : adding {len(files)} files')
...
def main():
    ...
    parser.add_argument('--replace', action='store_true', help='replace index file if it exists')
    ...
    index_for_glob(args.glob_string,
