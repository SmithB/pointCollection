...
class tiling(object):
    ...
    def _get_mapping_function(self, mapping_function_name=None):
    ...
    def to_json(self, json_file):
        ...
        scheme_dict = {field:getattr(self, field) for field in ['tile']}
    ...
    def tile_filename(self, xy_t):
        return os.path.join(self.directory, self.format_str % tuple(xy_t))+self.extension

    def filenames_for_xy(self, xy0):
        if np.isscalar(xy0[0]):
            xy0=[np.array([xy0[0]]).ravel(), np.array([xy0[1]]).ravel()]
        if not isinstance(xy0[0], np.array):
